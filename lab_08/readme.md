...
0. Pracę wykonaj na nowym branchu.
1. Odtwórz w swojej aplikacji kolejne kroki przedstawione w labie aby uruchomić możliwość korzystania z GraphQL. Wspomagaj się materiałami z wykładu.
2. Dodaj jeszcze 3 dodatkowe resolvery pozwalające na odpytanie API za pomocą GraphQL, które pozwolą np. odfiltrować obiekty po fragmencie nazwy, wyświetlić ilość obiektów z daną wartością atrybutu (np. liczba postów dla danego użytkownika).
3. Dla modelu `Post` stwórz również mutacje pozwalające na tworzenie, edycję i usuwanie encji tego typu.

Zapytania GraphQL zapisz w pliku `lab_8_zadania.md` w folderze projektu na tym branchu.
