...
Do tej pory dostęp do API nie jest w żaden sposób kontrolowany i kążdy znając adres URL może nasze API odpytać, również dla żądań PUT, CREATE czy DELETE.
...
Aktualnie wszystkie metody (PUT, GET, DELETE) są zdefiniowane w jednej funkcji, co zabiera nam mozliwość rozdzielenia uprawnień dla każdego z nich.
Powinnismy więc najpierw rozdzielić oba widoki. Dla przypomnienia, poniżej oryginalny kod dla endpointów związanych z klasą Person.
...
Pojawienie się dekoratorów `@authentication_classes([SessionAuthentication, BasicAuthentication])` oraz `@permission_classes([IsAuthenticated])` określnia odpowiednio dopuszczalne metody autentykacji oraz listę klas, które definiują wymagane uprawnienia. Bez ich spełnienia kod metody nie zostanie wykonany, a do obiektu `Response` zostanie dołączony odpowiedni kod błędu np.:
```HTTP 403 Forbidden
...
```
Dodatkowu musimy również zaktualizować definicję URL-i dla zmodyfikowanych endpointów. Postać pliku `ankiety/urls.py` po modyfikacji:
```python
...
```
Docelowo niezłym pomysłem jest budowanie oddzielnego endpointu dla każdej operacji, co zmniejszy ilość refactoringu, który będzie trzeba wykonac w przyszłości jeżeli pojawi się potrzeba dodania uprawnień dla każdego z nich z osobna.
...
Po odświeżeniu widoku API w prawym rogu pojawi się możliwość zalogowania z wykorzystaniem wcześniej utworzonych kont użytkowników. Po uwierzytelnieniu otrzymamy dostęp do wszystkich endpointów, które wymagają uprawnienia `IsAuthenticated`.
...
5. Dodaj lub zmodyfikuj istniejący endpoint tak aby możliwe było wyświetlenie wszystkich obiektów typu Osoba przypisanych do danego id drużyny. Niech url dla niego będzie postaci `.\druzyna\<id>\czlonkowie\`. Niech dostęp będzie tylko do odczytu i tylko poprzez autoryzację tokenem.